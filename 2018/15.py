...
class Grid(dict):
    ...
    def print(self):
    	G = [['#' if self[Pt(i, j)] else '.' for j in range(self.w)] for i in range(self.h)]
    	for u in self.units:
    		if u.alive:
	    		G[u.position[0]][u.position[1]] = 'E' if u.team == Team.ELF else 'G'
    ...
    def play(self, elf_death=False):
        ...
        while True:
            ...
            # self.print()
            rounds += 1
        print(rounds, sum(unit.hp for unit in self.units if unit.alive))
...
lines = open('15_input').read().splitlines()
