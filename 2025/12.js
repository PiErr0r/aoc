...
function part1(data) {
	...
	let res = 0;
	data = getGroups(data);
	const trees = data.splice(data.length - 1, 1);
	const presents = data.map(p => count(p, '#'));
	iter(trees[0].split('\n'), t => {
		const [sz, cnt] = t.split(': ');
		const [w, h] = sz.split('x').map(n => int(n));
		const cnts = cnt.split(' ').map(n => int(n));
		res += sum(cnts.map((c, i) => c * presents[i])) <= w * h;
	})
